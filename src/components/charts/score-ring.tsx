...
export function ScoreRing({ score, size = 132, strokeWidth = 10, label, className }: Props) {
  ...
  const color =
    clamped >= 80 ? "hsl(var(--profit))" : clamped >= 60 ? "hsl(var(--warning))" : "hsl(var(--loss))";
  const textColor =
    clamped >= 80 ? "text-profit" : clamped >= 60 ? "text-warning" : "text-loss";
...
export function ScoreMeter({ label, score, detail }: { label: string; score: number; detail?: string }) {
  const color = score >= 80 ? "bg-profit" : score >= 60 ? "bg-warning" : "bg-loss";
  return (
    <div>
